...
- **Upload screen**: Drag-and-drop or file picker for two images (A = left, B = right). Drop on a specific box or anywhere on the screen (auto-fills A then B). Upload boxes dynamically resize to match image aspect ratio via `normalizeUploadBoxes()` — both boxes always share the same dimensions (widest aspect ratio wins).
